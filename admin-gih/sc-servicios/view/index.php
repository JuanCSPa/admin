...
include "../model/servicios.class.php";
      ...
      <div class="box box-primary">
      	<div class="box-body">
        	<?php foreach ($datos as $value) { ?>
          		...
          		<img src="../img/<?=$value['ser_image']?>" class="img-responsive img-rounded">
          		<br>
							<a href="#" data-toggle="modal" data-target="#modalForm<?=$value['ser_id']?>" class="btn btn-primary"><i class="fa fa-pencil-square-o"></i> Editar</a>
							<a class="btn btn-danger" href="#" onclick="javascript:eliminarArchivo(<?=$value['ser_id']?>)"><i class="fa fa-trash-o"></i> Eliminar</a>
							<div id="msgBoxEliminar<?=$value['ser_id']?>"></div>

											<!--inicio modal formulario Edit-->
															<!-- Modal -->
															<div class="modal fade" id="modalForm<?=$value['ser_id']?>" role="dialog">
															    <div class="modal-dialog">
															    	<form role="form" action="#" name="frm_edicion<?=$value['ser_id']?>" id="frm_edicion<?=$value['ser_id']?>" method="post" enctype="multipart/form-data">
																        <div class="modal-content">
																            ...
																            <div class="modal-header" style="background: #c8241b;">
																                ...
																                <h4 class="modal-title" style="color: #fff; text-align: left" id="myModalLabel">Edición Servicios</h4>
																            </div>
																            ...
																            <div class="modal-body">
																                ...
																                <input class="form-control" name="nombre"  placeholder="Nombre del Servicio" value="<?=$value['ser_titulo']?>">
																											<hr>
																											<label>Descripcion del Servicio</label>
																											<textarea name="desclar<?=$value['ser_id']?>" class="form-control" id="desclar<?=$value['ser_id']?>" rows="2" cols="80" placeholder="Descripcion Larga del Producto"><?=$value['ser_content']?></textarea>
																											<script>
																												CKEDITOR.replace("desclar<?=$value['ser_id']?>");
																											</script>
																				<h5><span class="label label-warning">Imagen Servicio (580 x 500)px</span></h5>
																                <input type="file" name="imagen" id="imagen<?=$value['ser_id']?>" class="file">
																                <input type="hidden" value="<?=$value['ser_image']?>" name="actual">
																                <input type="hidden" value="<?=$value['ser_id']?>" name="idservicios">

																                <script type="text/javascript">
																							   $("#imagen<?=$value['ser_id']?>").fileinput(
																							   {
																							   		autoReplace: true,
																								    ...
																								    initialPreview:[
																								    	"<img class='kv-preview-data file-preview-image img-responsive img-rounded img-thumbnail' width='40%' src='../img/<?=$value['ser_image']?>'>"
																								    ],
																								    layoutTemplates: {actionDelete: ''},
																								});
																							</script>
																				<div id="msgBox<?=$value['ser_id']?>"></div>
																            </div>

																            <!-- Modal Footer -->
																            <div class="modal-footer">
																                <button type="button" class="btn btn-danger" data-dismiss="modal">Cerrar</button>
																              <button type="button" class="btn btn-primary" style="background: #2d3e52; border-color: transparent;" onclick="enviarInformacion(<?=$value['ser_id']?>)">Guardar</button>
																            </div>
																        </div>
																    </form>
															    </div>
															    	...
															    	<form role="form" action="#" name="frm_subir" id="frm_subir" method="post" enctype="multipart/form-data">
																        <div class="modal-content">
																            ...
																            <div class="modal-header" style="background: #c8241b;">
																                ...
																                <h4 class="modal-title" style="color: #fff; text-align: left" id="myModalLabel">Nueva Servicio</h4>
																            </div>

																            <!-- Modal Body -->
																            <div class="modal-body">
																                <label>Nombre del Servicio</label>
																                <input class="form-control" name="nombre" placeholder="Nombre de Servicio" value="">
...
<script>
	...
	function subirInformacion(){
		...
		$.ajax({
			url:'../control/servicios.insert.php',
			type:'POST',
			data: formData,
			cache: false,
			contentType: false,
    ...
    function enviarInformacion($id){
	    ...
	    $.ajax({
	        url: '../control/servicios.update.php',
	        type: 'POST',
	        data: formData,
	        cache: false,
	        contentType: false,
	...
	function eliminarArchivo($id){
		...
		$.ajax({
			url:'../control/servicios.delete.php',
			type:'get',
			data:{'servicios':$id},
			beforeSend: function(){
