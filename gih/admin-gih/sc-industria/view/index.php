...
include "../model/industria.class.php";
      ...
      <div class="box box-primary">
      	<div class="box-body">
        	<?php foreach ($datos as $value) { ?>
          		...
          		<img src="../img/<?=$value['ind_image']?>" class="img-responsive img-rounded">
          		<br>
			<a href="#" data-toggle="modal" data-target="#modalForm<?=$value['ind_id']?>" class="btn btn-primary"><i class="fa fa-pencil-square-o"></i> Editar</a>
			<a class="btn btn-danger" href="#" onclick="javascript:eliminarArchivo(<?=$value['ind_id']?>)"><i class="fa fa-trash-o"></i> Eliminar</a>
			<div id="msgBoxEliminar<?=$value['ind_id']?>"></div>

			<!--inicio modal formulario Edit-->
							<!-- Modal -->
							<div class="modal fade" id="modalForm<?=$value['ind_id']?>" role="dialog">
							    <div class="modal-dialog">
							    	<form role="form" action="#" name="frm_edicion<?=$value['ind_id']?>" id="frm_edicion<?=$value['ind_id']?>" method="post" enctype="multipart/form-data">
								        <div class="modal-content">
								            ...
								            <div class="modal-body">
								                ...
								                <input class="form-control" name="nombre"  placeholder="Nombre del la Industria" value="<?=$value['ind_titulo']?>">
								                <label>Icono</label>
								                <textarea name="icono<?=$value['ind_id']?>" rows="5" class="form-control borderColor" placeholder="Contenido Apartado Cuatro"><?=$value['ind_icono']?></textarea>
								                <script>
								                	CKEDITOR.replace("icono<?=$value['ind_id']?>");
								                </script>
												<h5><span class="label label-warning">Imagen Industria (500 x 600)px</span></h5>
												<hr>
								                <input type="file" name="imagen" id="imagen<?=$value['ind_id']?>" class="file">
								                <input type="hidden" value="<?=$value['ind_image']?>" name="actual">
								                <input type="hidden" value="<?=$value['ind_id']?>" name="idind">

								                <script type="text/javascript">
															   $("#imagen<?=$value['ind_id']?>").fileinput(
															   {
															   		autoReplace: true,
																    ...
																    initialPreview:[
																    	"<img class='kv-preview-data file-preview-image img-responsive img-rounded img-thumbnail' width='40%' src='../img/<?=$value['ind_image']?>'>"
																    ],
																    layoutTemplates: {actionDelete: ''},
																});
															</script>
												<label>URL</label>
											<input type="text" name="url" placeholder="Ejemplo: http://www.google.com.mx" class="form-control" id="url" value="<?=$value['ind_url']?>">
												<div id="msgBox<?=$value['ind_id']?>"></div>
								            </div>

								            <!-- Modal Footer -->
								            <div class="modal-footer">
								                <button type="button" class="btn btn-danger" data-dismiss="modal">Cerrar</button>
								              <button type="button" class="btn btn-primary" style="background: #2d3e52; border-color: transparent;" onclick="enviarInformacion(<?=$value['ind_id']?>)">Guardar</button>
								            </div>
								        </div>
								    </form>
							    </div>
			    	...
			    	<form role="form" action="#" name="frm_subir" id="frm_subir" method="post" enctype="multipart/form-data">
				        <div class="modal-content">
				            ...
				            <div class="modal-header" style="background: #c8241b;">
				                ...
				                <h4 class="modal-title" style="color: #fff; text-align: left" id="myModalLabel">Nueva Industria</h4>
				            </div>

				            <!-- Modal Body -->
				            <div class="modal-body">
				                <label>Nombre de la Industria</label>
				                <input class="form-control" name="nombre" placeholder="Nombre de la Industria" value="">
				                <label>Agregar Icono</label>
								<textarea name="icono" rows="5" class="form-control borderColor" placeholder="Contenido Apartado Cuatro"></textarea>
								                <script>
								                	CKEDITOR.replace("icono");
								                </script>
				                <hr>
								<h5><span class="label label-warning">Imagen Industria (500 x 600)px</span></h5>
...
<script>
	...
	function subirInformacion(){
		...
		$.ajax({
			url:'../control/industria.insert.php',
			type:'POST',
			data: formData,
			cache: false,
			contentType: false,
    ...
    function enviarInformacion($id){
	    ...
	    $.ajax({
	        url: '../control/industria.update.php',
	        type: 'POST',
	        data: formData,
	        cache: false,
	        contentType: false,
	...
	function eliminarArchivo($id){
		...
		$.ajax({
			url:'../control/industria.delete.php',
			type:'get',
			data:{'industria':$id},
			beforeSend: function(){
