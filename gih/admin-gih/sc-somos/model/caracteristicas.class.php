	...
	class caracteristicas
	{
		...
		function insert($array){
			if($this->con->conectar()==true){
				...
				$sql = "INSERT INTO tbl_caracteristicas(".$campos.") VALUES (".$datos.")";
		...
		function getById($id){
			if($this->con->conectar()==true){
				$sql = "SELECT * FROM tbl_caracteristicas WHERE car_id = %d;";
				$sql = sprintf($sql, $id);
				$data	= $this->Execute(mysql_query($sql));
				return $data;
			}
		...
		function update($array,$condicion){
			if($this->con->conectar()==true){
				...
				$sql = "UPDATE tbl_caracteristicas SET ".$valores ." WHERE ".$condicion;
				$bool = mysql_query($sql);
				return $bool;
			}
		}
		...
		function delete($id){
			if($this->con->conectar()==true){
				$sql = "DELETE FROM tbl_caracteristicas WHERE car_id = %d;";
				$sql = sprintf($sql, $id);
				$bool	= mysql_query($sql);
				return $bool;
			}
